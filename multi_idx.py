...
def run_experiment():
    ...
    # Generate orthonormal w_stars
    w_stars = []
    for _ in range(m):
        w = np.random.randn(d)
        w /= np.linalg.norm(w)
        # Orthogonalize against previous
        for prev_w in w_stars:
            w -= np.dot(w, prev_w) * prev_w
        w /= np.linalg.norm(w)
        w_stars.append(w)

    # Compute y as sum of relu3 over multiple indexes
    y = np.zeros(n)
    ...
    # Storage
    alpha_sgd = [np.sqrt(sum(np.dot(w_sgd, ws)**2 for ws in w_stars))]
    alpha_eos = [np.sqrt(sum(np.dot(w_eos, ws)**2 for ws in w_stars))]
    v_sgd_norms = []
    v_eos_norms = []

    print(f"Initial subspace alignment: {alpha_sgd[0]:.6f}")
    ...
    for step in range(steps):
        ...
        # Record
        alpha_sgd.append(np.sqrt(sum(np.dot(w_sgd, ws)**2 for ws in w_stars)))
        alpha_eos.append(np.sqrt(sum(np.dot(w_eos, ws)**2 for ws in w_stars)))
...
# Left subplot: Subspace Alignment
ax1.plot(time_alpha, alpha_sgd, color=sgd_color, linewidth=3,
         label='SGD Subspace Alignment', alpha=0.9)
ax1.plot(time_alpha, alpha_eos, color=eos_color, linewidth=3,
         label='EoS Subspace Alignment', alpha=0.9)
ax1.set_xlabel('Step', fontsize=14, fontweight='bold')
ax1.set_ylabel(r'Subspace Alignment $\alpha$', fontsize=14, fontweight='bold')
ax1.set_title('Subspace Alignment Across Steps', fontsize=16, fontweight='bold', pad=20)
...
plt.tight_layout()
plt.savefig('multi_index_eos_sgd_comparison.png', dpi=300, bbox_inches='tight',
